...
PubSubClient::PubSubClient(uint8_t *ip, uint16_t port, CallbackListener* cListener,Client& client){
   this->_client = &client;
   this->callbackListener = cListener;
   this->callback = NULL;
   this->ip = ip;
   this->port = port;
   this->domain = NULL;
         ...
         if (len > 0) {
            ...
            if (type == MQTTPUBLISH) {
			   ...
			   if(callbackListener){
                  ...
                  // ignore msgID - only support QoS 0 subs
                  uint8_t *payload = buffer+llen+3+tl;
                  callbackListener->callback(topic,payload,len-llen-3-tl);
			   }
